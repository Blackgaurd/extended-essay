...
def run(seed, c1):
    ...
    clf = GDTClassifier(
        ...
        optimal_depth=4,
        fitness_evaluator=FitnessEvaluator(c1, 1 - c1, negative_quadratic),
    )
...
c1s = [0.1, 0.3, 0.5, 0.7, 0.9]

product = list(itertools.product(range(3), c1s))
...
y = [mean(training_time[c1]) for c1 in c1s]
ybars = [stdev(training_time[c1]) for c1 in c1s]

ax = plt.subplot(gs[0, 0])
plt.title("Training time")
plt.ylabel("Time (s)")
plt.xlabel("C1")
plt.errorbar(c1s, y, ybars, label="Training time")

# top right: test accuracy
y = [mean(testing_accuracy[c1]) * 100 for c1 in c1s]
ybars = [stdev(testing_accuracy[c1]) * 100 for c1 in c1s]

ax = plt.subplot(gs[0, 1])
plt.title("Test accuracy")
plt.ylabel("Accuracy (%)")
plt.xlabel("C1")
plt.errorbar(c1s, y, ybars, label="Test accuracy")

# bottom span: accuracy convergence
ax = plt.subplot(gs[1, :])

x = list(avg_accuracy[c1s[0]].keys())
...
plt.savefig("figs/fitness_constants.png")
